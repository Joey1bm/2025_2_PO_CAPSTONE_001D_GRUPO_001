...
def create_app(config_class=None):
    ...
    if config_class:
        ...
        print(f"✅ Configuración cargada: {config_class.__name__}")
    else:
...
if __name__ == '__main__':
    ...
    if os.environ.get('FLASK_ENV') == 'production' or os.environ.get('RAILWAY_ENVIRONMENT'):
        app.config.from_object('config.ProductionConfig')
        print("🚀 Usando configuración PRODUCCIÓN")
    else:
        app.config.from_object('config.DevelopmentConfig') 
        print("🔧 Usando configuración DESARROLLO")
    ...
    try:
        ...
        app.logger.error(f"Error al iniciar servidor: {e}")
        sys.exit(1)

    @app.route('/debug')
    def debug_info():
        return {
            "status": "✅ Aplicación funcionando",
